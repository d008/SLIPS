...
yplus = water.nu/fac.u_tau;

%Calculate friction Re
Re_tau = [Re_tau;fac.u_tau*fac.delta/water.nu];

%Viscosity Ratio mu_water/mu_oil
N = water.mu/lub.mu;

%Calculate retention length [m] (zero laplace, chemical barrier)
[L1a , L2a] = retentionLength(lub.contAngle,surface.w*surface.aspectRatio,surface.w,lub.gamma,water.rho*fac.u_tau^2);
...
%Store shear
tau_w = [tau_w;water.rho*fac.u_tau^2]; 
...
We_tau = [We_tau; water.rho*fac.u_tau^2*surface.w/lub.gamma];

%Calculate Bond Number
Bo = [Bo;(water.rho - lub.rho)*(9.8)*(surface.w).^2./lub.gamma];
...
DR= [DR;getDragReduction(b_plus(end),B_t(surface.a,N)/surface.a*surface.w/yplus,fac.u_tau*fac.delta/water.nu)];

U_slip1 = [U_slip1;(100-DR(end))./100.*tau_w(end)./water.mu.*b(end)];
U_slip2 = [U_slip2;DR(end)/100*fac.U_inf];

%From the drag reduction approximate the slip velocity and estimate the
%slip Weber number
We_slip = [We_slip;water.rho*(U_slip1(end))^2*surface.w/lub.gamma];
